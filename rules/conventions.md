...
- The pipeline is thin on purpose: all logic lives in **`ops/cd-deploy.sh`**, which verifies the checkout's identity, snapshots local state, pins the checkout to one commit, runs `deploy.sh` with `DEPLOY_SKIP_PULL=1`, and smoke-tests `/api/health`, `/capture/health` and `/` through the proxy. Run it by hand when Actions is unavailable and you get the same deploy. Migrations run on api boot as part of it. First production release through this pipeline: `71ffbe2`, 2026-07-29.

### Writing CI jobs — the shared-runner rules
- **Never use a job-level `container:`.** Every job on the self-hosted runner shares one workspace (`_work/<repo>/<repo>`), and a `container:` job runs as **root** — its checkout leaves root-owned files that the next job, running as the `admin` service user, cannot delete, so `actions/checkout` dies with `EACCES` before a single step runs. A job authored this way *passes* and takes down every job scheduled after it. Run the image with `docker run` in a normal step instead (see the `capture-tests` and `nginx-config` jobs), mount the source **read-only**, and keep all writes inside the container (`PYTHONPYCACHEPREFIX`, pytest's `-p no:cacheprovider`).
- **Verify every workflow command locally before pushing it.** Run the exact `docker run …` line by hand first. Note the limit of that check, though: it cannot catch cross-job interactions like the workspace-ownership one above, which only exist on the shared runner.
- Green locally ≠ green in Actions. A job's own success says nothing about what it did to the workspace — after adding a job, check that the *following* jobs still pass.

### When CI hangs
- The runner can sit **connected, token-refreshing, and accepting no work**. Symptom: jobs stay `queued` for minutes while `systemctl is-active` says `active`, the listener logs say `Listening for Jobs`, and no `_diag/Worker_*.log` appears. Nothing looks unhealthy.
- Fix: `sudo systemctl restart actions.runner.papoveB01-ghoststream_project.ghoststream-ci.service`. Jobs are picked up within ~seconds. Reach for this **before** suspecting Actions billing or permissions — a wedged listener is both more common and cheaper to rule out.
- A queue of more than ~2 minutes against an idle runner is already abnormal; normal pickup is seconds.
